...
void IOManager::plot_solutions(std::string output_name, Method * analytical, Method * method, int number_processes) {
	...
	gp << "set key on box; set tics scale 0; set border 3; set ylabel \"Temperature [F]\";set xlabel \"x [ft]\"; set yrange [90:310]; set term png; set xtics (\"0\" 0, \"0.5\"" << cols / 2 << ", \"1\"" << cols - 1 << ")\n";
