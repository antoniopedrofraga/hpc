...
void Matrix::set_row(int index, Vector v) {
	...
	if (index < 0 || index >= (int)vector_size) throw std::out_of_range("index out of range.");
	if ((int)vector_size != (*this).getNcols()) throw std::out_of_range("vector size is different from matrix columns number.");
