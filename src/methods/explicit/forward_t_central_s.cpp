...
double* FTCS::build_iteration(double* current_step, double* previous_step, MPImanager *mpi_manager, double &back, double &forward) {
	...
	for (size_t i = 0; i <= size; i++) {
		wait(mpi_manager, i);
		//std::cout << "Rank = " << rank << " back = " << back << " forward = " << forward << std::endl;
...
void FTCS::wait(MPImanager * mpi_manager, size_t i) {
	...
	if (i == 0) {
		//cout << "Rank = " << rank << " sent forward to " << (rank - 1) << " Request Status 0 " << (request_status[0] ? "true" : "false") << endl;
		if (!mpi_manager->is_root() && request_status[0]) {
			MPI_Wait(&requests[0], MPI_STATUS_IGNORE);
			//std::cout << "Rank = " << rank << " sent forward to " << (rank - 1) << std::endl;
		}
		//cout << "Rank = " << rank << " received forward from " << (rank + 1) << " Request Status 1 " << (request_status[1] ? "true" : "false") << endl;
		/*if (!mpi_manager->is_last() && request_status[1]) {
			MPI_Wait(&requests[1], MPI_STATUS_IGNORE);
			std::cout << "Rank = " << rank << " received forward from " << (rank + 1) << std::endl;
		}*/
	}

	if (i == size) {
		//cout << "Rank = " << rank << " sent back to " << (rank + 1) << " Request Status 2 " << (request_status[2] ? "true" : "false") << endl;
		if (!mpi_manager->is_last() && request_status[2]) {
			MPI_Wait(&requests[2], MPI_STATUS_IGNORE);
			//std::cout << "Rank = " << rank << " sent back to " << (rank + 1) << std::endl;
		}
		//cout << "Rank = " << rank << " received back from " << (rank - 1) << " Request Status 3 " << (request_status[2] ? "true" : "false") << endl;
		/*if (!mpi_manager->is_root() && request_status[3]) {
			std::cout << "Testing Request 3 with flag" << std::endl;
			MPI_Wait(&requests[3], MPI_STATUS_IGNORE);
			std::cout << "Rank = " << rank << " received back from " << (rank - 1) << std::endl;
		}*/
	}
}
...
void FTCS::exchange_data(MPImanager *mpi_manager, size_t i, const double result[], double &back, double &forward) {
	...
	if (i == 0) {
		...
		if (!mpi_manager->is_last()) {
			MPI_Irecv(&forward, 1, MPI_DOUBLE, 0, rank + 1, MPI_COMM_WORLD, &requests[1]);
	...
	if (i == size) {
		...
		if (!mpi_manager->is_root()) {
			MPI_Irecv(&back, 1, MPI_DOUBLE, 0, rank - 1, MPI_COMM_WORLD, &requests[3]);
